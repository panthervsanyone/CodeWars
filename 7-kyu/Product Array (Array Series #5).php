...
Given an array/list [] of integers , Construct a product array Of same
 size Such That prod[i] is equal to The Product of all the elements of Arr[] except Arr[i].
...
function productArray($nums):array
{
    $finalRes = [];
    $countNum = count($nums);
    foreach ($nums as $key => $value) {
        ...
        for ($i = 0; $i <= $countNum; $i++) {
            if ($key == $i) {
                ...
                $finalRes []= $resultForMultiply;
                break;
            }

            $resultForMultiply *= $nums[$i];
