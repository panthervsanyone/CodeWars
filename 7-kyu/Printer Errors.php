...
of the printer as a string representing a rational whose numerator is the number of errors and the denominator
...
function printerError($s) {
    ...
    $countLine = strlen($s);
    $counter = 0;
    $arrOfValue = str_split($s);
    foreach ($arrOfValue as $val){
        if (!in_array($val, $valid))
            $counter++;
    }
    return $counter.'/'.$countLine;
}