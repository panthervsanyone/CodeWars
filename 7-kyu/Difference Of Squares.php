...
Hence the difference between square of the sum of the first ten natural numbers and the sum
of the squares of those numbes is: 3025 - 385 = 2640
 */

function difference_of_squares(int $n): int
{
    $listNumbers = range(1, $n);
    $sumAllPow = array_sum($listNumbers) ** 2;
    $sumPow = 0;
    foreach ($listNumbers as $value) {
        $sumPow += $value ** 2;
    }
    return $sumAllPow - $sumPow;
}