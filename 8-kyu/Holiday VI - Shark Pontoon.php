...
as you do the mental calculations in the water you either freeze when you realise you are dead, or
swim when you realise you can make it!

You are given 5 variables;

sharkDistance = distance from the shark to the pontoon. The shark will eat you if it
reaches you before you escape to the pontoon.
...
{
    $res = $dolphin ? $sharkSpeed / 2 : $sharkSpeed;
    $humanTime = $pontoonDistance / $youSpeed;
    $sharkTime = $sharkDistance / $res;
    return $humanTime < $sharkTime ? "Alive!" : "Shark Bait!";
}

function shark(int $pontoonDistance, int $sharkDistance, int $youSpeed, int $sharkSpeed, bool $dolphin): string {
    $res = $dolphin ? $sharkSpeed / 2 : $sharkSpeed;
    if(($pontoonDistance / $youSpeed) < ($sharkDistance / $res)) {
        return "Alive!";
    }
    return 'Shark Bait!';
}