...
it. If the character code of any of the characters in the string matches the number, you get a mini win.
...
function bingo(array $ticket, int $win):string {
    $counter = 0;
    foreach ($ticket as $arrCheck) {
        $split = str_split($arrCheck[0]);
        foreach ($split as $value) {
            if (ord($value) == $arrCheck[1]) {
                $counter++;
                break;
            }
        }
